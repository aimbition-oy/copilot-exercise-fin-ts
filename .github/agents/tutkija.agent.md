...
syntetisoida ja raportoida tietoa, jotta käyttäjä (tai kutsuva agentti) voi tehdä
perustellun päätöksen tai siirtää eteenpäin hyvin rajatun toteutustehtävän.
...
1. `README.md`, `PRD.md`, `EXERCISES.md` ja muut `.md`-suunnitelmatiedostot — tarkista aina ensin.
2. `.github/**` — ohjeet, agentit, skillit, hookit.
3. `src/**` backend-kysymyksiin (kerrostus: routes → services → repositories → models → data).
4. `public/**` frontend-kysymyksiin, `tests/**` käyttäytymiskysymyksiin — testit
   dokumentoivat agronomiset säännöt täsmällisesti.

Älä lue tai paljasta tiedostoja työtilan juuren ulkopuolelta, ja käsittele
`.env*`-tiedostoja arkaluonteisina — viittaa niiden olemassaoloon, mutta älä
koskaan lainaa sisältöä.
...
3. Kerää konteksti rinnakkain, kun mahdollista: työtilan haku/luku repo-kysymyksiin, web-haku ulkoisiin dokumentteihin, molemmat kun vertaat repoa ylävirran ohjeisiin.
...
- **normaali** _(oletus)_: laajempi pyyhkäisy olennaisesta alueesta (backend, frontend, testit tai harness). Jäsennelty raportti: löydökset + 1 suositus.
